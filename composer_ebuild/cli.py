...
class ComposerEbuildGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            temp_dir: Temporary directory for package installation
            config: Configuration object with debug, github_token and skip_downgrade settings
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self._install_composer_package()
    ...
    def _install_composer_package(self) -> None:
        """
        Install the Composer package in a temporary directory and lock its version.

        Raises:
            ComposerPackageInstallError: If the installation fails
        ...
        return_code, stdout, _ = run_subprocess(show_command, capture_output=True)
        if return_code != 0:
    ...
    def _update_composer_dependencies(self) -> None:
        """
    ...
    def _generate_ebuilds(self) -> None:
    ...
    def _assign_package_dependencies(
        ...
        dependencies: list[dict[str, str | list[dict]]],
    ) -> None:
        """
        Assign dependencies to a package.

        Args:
            package: The ComposerPackage instance to assign dependencies to
            dependencies: List of dependencies from the composer show output with name and optional requires fields
    ...
    def _process_dependencies(self, package: ComposerPackage) -> None:
        ...
        return_code, stdout, stderr = run_subprocess(command, cwd=self.temp_dir, capture_output=True)

        if return_code == 0:
            dependency_tree = json.loads(stdout)
            ...
            if "requires" in main_package:
                self._assign_package_dependencies(package, main_package["requires"])
            else:
